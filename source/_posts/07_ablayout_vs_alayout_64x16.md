---
title: ABLayout vs ALayout_64x16 深度对比 - SS 与 RS 模式的 TV Layout
date: 2024-12-24
categories: [CUDA, CuTe, CUTLASS]
tags: [layout, wgmma, ss-mode, rs-mode, tv-layout]
---

# ABLayout vs ALayout_64x16 深度对比

## 1. 引言

在 CUTLASS 的 WGMMA (Warpgroup Matrix Multiply-Accumulate) 中，有两种操作数访问模式：
- **SS 模式**: A 和 B 都在 Shared Memory，使用 `ABLayout`
- **RS 模式**: A 在 Register，B 在 Shared Memory，使用 `ALayout_64x16`

本文深入对比这两种 TV Layout 的结构、映射函数和语义差异。
...
Thread Stride = 0 意味着:
  ...
  offset(thread_1,   v_m, v_k) = v_m + v_k × 64   ← 相同!
  offset(thread_2,   v_m, v_k) = v_m + v_k × 64   ← 相同!
  ...
  offset(thread_127, v_m, v_k) = v_m + v_k × 64   ← 相同!

┌─────────────────────────────────────────────────────────────┐
│          所有 128 个线程计算出相同的 offset！               │
│                                                             │
│          它们共享同一块 SMEM 数据                           │
└─────────────────────────────────────────────────────────────┘
```

### 3.5 64×16 矩阵的 Offset 分布

```
64×16 矩阵 (Column-Major, 所有线程共享):
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  GMEM ──TMA──→ SMEM ──descriptor──→ WGMMA 硬件             │
│                  ↑                                          │
│           64-bit descriptor                                 │
│           指向 SMEM 起始地址                                │
│                  ↑                                          │
│         所有 128 线程共享                                   │
│                                                             │
│  特点: 数据在 SMEM，硬件直接读取，无需 load 到 register   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
Thread 0 负责:
  - 4 个位置在 m=0 行: (0,0), (0,1), (0,8), (0,9)
  - 4 个位置在 m=8 行: (8,0), (8,1), (8,8), (8,9)
```
...
观察规律:
  - k 方向: T0-T3 交替，每个 thread 覆盖 2 列
  - m 方向: 相同 thread 每隔 8 行重复
  - 每个 thread 的 8 个值分散在 4 个 2×2 小块中
```
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  GMEM ──TMA──→ SMEM ──ldmatrix──→ Register ──→ WGMMA 硬件  │
│                          ↑            ↑                     │
│                    每线程 load    每线程 8 个值            │
│                    自己的 8 个值  存在 register            │
│                          ↑                                  │
│                  ALayout_64x16 指定                        │
│                  每线程负责哪些位置                         │
│                                                             │
│  特点: 数据需要从 SMEM load 到 Register                    │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Thread 0 ──→ SMEM descriptor ──→ 访问整个 64×16           │
│  Thread 1 ──→ SMEM descriptor ──→ 访问整个 64×16  (相同!)  │
│  Thread 2 ──→ SMEM descriptor ──→ 访问整个 64×16  (相同!)  │
│  ...                                                        │
│  Thread 127 → SMEM descriptor ──→ 访问整个 64×16  (相同!)  │
│                                                             │
│  数据位置: SMEM (不移动)                                   │
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Thread 0 ──→ Register[0..7] ──→ 位置 (0,0)(0,1)(8,0)...   │
│  Thread 1 ──→ Register[0..7] ──→ 位置 (0,2)(0,3)(8,2)...   │
│  Thread 2 ──→ Register[0..7] ──→ 位置 (0,4)(0,5)(8,4)...   │
│  ...                                                        │
│  Thread 127 → Register[0..7] ──→ 各自不同的 8 个位置       │
│                                                             │
│  数据位置: Register (需要从 SMEM load)                     │
│  Engine: ArrayEngine<uint32_t, 4> (寄存器数组)             │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
```
 ...
 Thr Stride        │      0      │                   │    ≠ 0      │
                   ...
                   └──────┬──────┘                   └──────┬──────┘
                          ...
                          ▼                                 ▼
                 ┌────────────────┐               ┌────────────────┐
 Thread 视图     │ 所有线程看到   │               │ 每个线程看到   │
                 │ 相同的 offset  │               │ 不同的 offset  │
                 └────────┬───────┘               └────────┬───────┘
                          │                                 │
                          ▼                                 ▼
                 ┌────────────────┐               ┌────────────────┐
